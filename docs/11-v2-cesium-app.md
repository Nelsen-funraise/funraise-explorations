...
### 20.4 線上版 server：Vercel 一鍵部署（`app/api/index.mjs`、`app/vercel.json`、`server/statedir.mjs`、`docs/12-hosting.md`）

Pages 只能放靜態前端；AI 模式、Fish 語音、FUNRAISE MCP 即時資料要有 `server/index.mjs`。這一批把 server 改成「可被 import 的 handler」：`export const handler`（就是本機 `createServer` 的那支 callback），只有直接執行時才 `.listen()`（`PEAKLENS_NO_LISTEN=1` 可關）；`app/api/index.mjs` 把它掛成 Vercel Node function，`vercel.json` 用 rewrites 把 `/api/(.*)` 導到它（官方 Express-on-Vercel 的同一招，函式收到的 `req.url` 仍是原始路徑），`functions.includeFiles` 把 `public/data/**` 帶進函式讓 `snapshot.mjs` 讀得到快照（它的資料目錄解析也改成多候選）。唯讀檔案系統的對策：`resolveStateDir()` 實際試寫，不可寫就退到 `os.tmpdir()/peaklens`（token 檔、TTS 快取、即時資料 24h 快取都走它）；MCP token 用 `FUNRAISE_MCP_TOKEN_JSON` 環境變數種子（本機 `/setup` 新增「匯出 MCP token」一鍵複製），`/api/health` 回報 `mcp.persist: file|env|tmp|none`，refresh 在 tmp 上輪替時給 `mcp.warning` 提醒重貼；`PUBLIC_URL` 缺省時由 `VERCEL_PROJECT_PRODUCTION_URL`／`VERCEL_URL` 推出 OAuth redirect。`npm run test` 跑四份 server 測試（快照 74、即時 35、生活圈 40、handler 18）。部署步驟、環境變數表（哪把金鑰解鎖什麼、去哪拿）、把 Pages 前端指向 Vercel API 的兩種方法（repo Variable `PEAKLENS_API_BASE` + `ALLOWED_ORIGIN`，或 🔑 金鑰對話框／`?api=&code=`）、Docker／Cloud Run 替代與安全注意事項，全部在 `docs/12-hosting.md`。尚未在真實 Vercel 上驗證：串流是否逐段到達、Hobby 方案的 `maxDuration`。

### 20.2 效能與「先 pre-render 好」（`src/prewarm.js`、`scenes.js`、`layers/photoreal.js`、`compose.js`、`viewer.js`）

- **開機／開播前預熱**：Cesium 沒有「預載某個視角的 tiles」的 API（`preloadFlightDestinations` 只在飛行途中對目的地生效），但 Google 相片級 tileset 的記憶體快取（預設 1.5 GiB）會留住最近幾個畫面用過的 tile。所以 `prewarmViews()` 趁 `#loading` 遮罩還蓋著，把相機依序瞬移到場景每一段的機位，各停到 `tileset.tilesLoaded`／`globe.tilesLoaded` 為止（每機位 ≤3.2 s、總共 ≤24 s），再放回原位。場景物件多了 `views`（`land` 七個機位；`compileScript()` 也會從有地點的段落自動產出），`SceneDirector.prewarm(scene)` 只在實景 tileset 顯示中才動作，五分鐘內預熱過就跳過；`play()` 開播前會再跑一次（把 `#loading` 當「場景準備中」遮罩，相機跳動看不到）。開機預設預熱 `land`（`?prewarm=<id|off>`、`localStorage pl.prewarm` 可改）。另外每一段結尾在實景下最多再等 2.5 s 讓當前畫面的 tile 補完，才飛下一段——「鏡頭帶過去了、畫面還糊」的兩個成因（快取裡沒有、飛太快）各堵一個。
- **旁白改用預先生成的 Nelsen 聲音**：線上版沒有 server 就沒有 `/api/tts`，只剩 `public/audio/<voice>/<fnv1a(speakable(text))>.mp3`。Phase 9F／10Q 之後多數旁白是 `(ctx) => string`（讀快照與 timeseries、依實景有無分支），舊 manifest 對不上就退回系統語音——這就是「聲音不是 Nelsen」的原因。`scripts/prerender-tts.mjs` 改成用 Node 端的 ctx（同一份 `peaklens.json`／`timeseries.json`、忠實重做的 `cityStats`／`topMover`／`metricLabel`、實景兩種變體）解析每段文字，再以 `speakable()` 正規化後雜湊（`src/speech-text.js` 是瀏覽器與 Node 共用的 sanitizer＋fnv1a）。本次 6 個場景 33 段旁白全部有 Nelsen 音檔（14 新生成、19 沿用），冒煙多了「narration-coverage」步驟：兩種變體下每段都必須命中 manifest。
- **實景串流與 GPU 負載（agent P，`layers/photoreal.js`、`compose.js`、`viewer.js`、`layers/funraise.js`）**：
  - tileset 選項明寫：`cacheBytes` 1.5 GiB／`maximumCacheOverflowBytes` 1 GiB（Cesium 1.124 `createGooglePhotorealistic3DTileset()` 的內建值，明寫避免版本變動）、`preloadFlightDestinations: true`、**`cullRequestsWhileMoving: false`**（預設 true 會在相機移動的整段飛行期間砍掉所有 tile 請求——連 preloadFlightDestinations 的也砍，這正是「鏡頭到了才開始載」的機制成因）、`dynamicScreenSpaceErrorFactor` 32、`skipScreenSpaceErrorFactor` 24、`skipLevels` 2；`immediatelyLoadDesiredLevelOfDetail` 維持 false（拿掉粗糙祖先 tile 會變成空洞，比模糊更糟）。
  - `RequestScheduler.maximumRequests` 50→96、`requestsByServer['tile.googleapis.com:443']` 18→24（ion 資產 2275207 與 Google 金鑰兩條路徑最終都從這個主機串流）。
  - 尺度 SSE 放寬：S4 16／S3 20／S2 28／S1 32（筆電上「準時但稍軟」勝過「更銳利但常常晚到」）；飛行／環繞中再乘 1.5（`photoreal.setMotion`，由 `camera.moveStart/moveEnd` 驅動，還原有 400 ms 去彈跳）；場景播放中 S4 用 S3 的值。
  - 實景顯示時 `scene.msaaSamples` 4→1、`globe.maximumScreenSpaceError` 2→3、`globe.tileCacheSize` 100→400（退出還原）；補上夜景借用實景底座時不再要求 bloom／HDR（`guardPhotorealQuality`）。
  - `#photoreal-status` 晶片改成狀態機：待載 tile > 30 且持續 > 800 ms 才出現，`body.cinema`／`presenting` 期間立即隱藏，退回門檻下 600 ms 淡出——不再整場閃。
  - CPU：`apply()` 尺度切換時標籤只重算一次（`setLabelBudget(n, {recompute:false})`）；`recomputeLabels()` 只掃 build 時就有標籤的實體快取（`_labeled`，時光機與 fx 兩個 datasource 仍即時掃）；高流量 `CallbackProperty` 材質改用預建的 `Color` 實例。無頭量測（SwiftShader，無 tileset）ScriptDuration 11.43→11.04 s／10 s 視窗，僅 CPU 端槓桿可量；串流與 GPU 槓桿要在真機＋金鑰下看。
  - 未修（不在該代理檔案範圍）：實景啟用中直接下「日照 17:30」會經 `ui.setSun` 重新開啟陰影疊在 tileset 上。
