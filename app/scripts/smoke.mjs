...
try {
  ...
  { const t2 = Date.now(); await page.goto('about:blank'); await page.goto(shareUrl, { waitUntil: 'domcontentloaded', timeout: 120000 }); // via about:blank: a hash-only change would not reload await page.waitForSelector('#loading.done', { timeout: 240000 }); for (let i = 0; i < 20 && !(await page.evaluate(() => !!window.PL)); i++) await wait(1500); await wait(2000);
