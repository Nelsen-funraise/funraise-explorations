...
export class Agent {
  ...
  async handle(text) {
    ...
    try {
      if (has(t, /^(幫助|help|你會什麼|可以問什麼)/)) return this.finish(a, '我聽得懂：「帶我去 ○○」、「切換 投資人／開發商／選址／城市／學研 視角」、「顯示／隱藏 都更／建照／上市公司交易」、「這裡容積率多少」、「2028 年南港會長出什麼」、「最近半年信義區上市公司買了什麼」、「比較 A 商圈和 B 商圈的租金」、「哪些公司遷入中山區」、「環繞／街景／俯視／全台／時光模式」、「夜視／熱感／藍圖」。或按「▶ 場景」看電影式導覽。');
  ...
  async deals(text, a) {
    ...
    this.ui.showBars('交易金額（前 6 筆）', list.slice(0, 6).map(m => ({ k: m.company_name, v: m.total_price || 0, label: fmtMoney(m.total_price) })), '#E05C3E'); return this.finish(a, lines.join('\n'));
  }
  async renewal(text, a) {
    ...
    if (stat.length) this.ui.showBars('各區都更件數', stat.slice(0, 8).map(s => ({ k: s.district, v: s.count, label: fmtInt(s.count) })), '#9A6FE0'); return this.finish(a, lines.join('\n'));
  }
  async compare(text, a) {
    ...
    if (areas.length >= 2) { await this.call(a, 'areas.list_areas', { city_code: 'A', type: 'business_area' }, () => ({ summary: `${areas.length} 商圈`, value: areas })); const rent = /租/.test(t) || !/售|房價|價/.test(t); const rows = areas.map(x => ({ k: (x.name || '').replace(/^台北市/, ''), v: rent ? x.market_price.actual_rent_avg : x.market_price.actual_sale_avg / 1e4, label: rent ? fmtInt(x.market_price.actual_rent_avg) + ' 元/坪/月' : fmtInt(x.market_price.actual_sale_avg / 1e4) + ' 萬/坪', yoy: rent ? x.market_price.actual_rent_yoy : x.market_price.actual_sale_yoy })); this.ui.showBars(rent ? '平均租金' : '平均售價', rows, '#C98E2C'); if (areas[0].lat) this.map.flyTo(areas[0].lon, areas[0].lat, { range: 7000, pitch: -58 }); this.ui.setLayer('heat', true);
