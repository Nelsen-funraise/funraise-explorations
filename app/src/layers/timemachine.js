...
// 資料來源：優先讀 public/data/timeseries.json（schema 見 TIMESERIES_README.md，12 區 × 2012–2026 年度計數 ×
// sales_all／sales_office／licenses 三個指標）；缺檔／格式不符時，自動退回用 peaklens.json 快照（mops＋buildings＋
// building_licenses）逐年逐區計數，保證這層永遠不會因為缺檔而整個炸掉。同時：MOPS 金額浮動標籤（S2–S3、2s 淡出）
// 與商圈熱區標籤的季度數字（2024 年起，讀 business_areas[].self_series），皆由這個模組在既有實體上「外掛」，
// 不改 funraise.js 的建圖邏輯。
//
// 資料品質但書（見 TIMESERIES_README.md）：
//  · sales_office 在 2012–2016 每個區都接近 0（實價登錄 main_use 早期標註涵蓋率問題，不是真的沒有交易）——
//    選這個指標時，2017 年以前一律顯示「資料涵蓋不足」（灰、不算年增率），避免看起來像 2017 年憑空暴增。
//  · 2026 是年初至今（YTD，meta.ytd_year），不是完整年度——HUD 與地圖標籤都標「2026 至今」，且不對它算年增率。
//  · licenses 混合新建／增建／修建／變更，一律稱「建照核發」，不稱「新建案」。
import * as Cesium from 'cesium';
import { fmtInt, fmtMoney } from './funraise.js';

const DEFAULT_YEARS = Array.from({ length: 15 }, (_, i) => 2012 + i); // 2012..2026
const DEFAULT_YTD_YEAR = 2026;
const METRICS = {
...
export class TimeMachine {
  constructor({ viewer, layers, data, basemap, timeline }) {
    ...
    this.metric = 'sales_all'; this._ts = null; this._src = 'snapshot'; this.years = DEFAULT_YEARS; this._ytdYear = DEFAULT_YTD_YEAR;
   ...
   * （即使沒有真正的年度統計檔，這層也要有「每年都在變」的內容，而不是全部掛零）。 */
  _buildFallback() {
  ...
  _excludedFromRange(metric, year) { const from = RELIABLE_FROM[metric]; if (from != null && year < from) return true; return year === this._ytdYear; }
  _computeRanges() { for (const metric of Object.keys(METRICS)) { let mn = Infinity, mx = -Infinity;
  ...
  _valueInfo(metric, name, year) {
    const idx = this._idxForYear(year); const y = this.years[idx] ?? year;
    const insufficient = RELIABLE_FROM[metric] != null && y < RELIABLE_FROM[metric];
    const ytd = y === this._ytdYear;
    const raw = this.seriesFor(metric, name)[idx] || 0;
    const yoy = (insufficient || ytd) ? null : pctNorm(this.yoyFor(metric, name, idx));
    const v = insufficient ? (this._range[metric] ? this._range[metric].min : 0) : raw; // 涵蓋不足年份：高度釘在該指標的正規化底線，不畫過小、會誤導的真實數字
    return { idx, year: y, raw, v, yoy, insufficient, ytd };
  }
  ...
  _labelText(name) {
    ...
    if (info.insufficient) return `${name} · 資料涵蓋不足`;
    const arrow = info.ytd ? `${info.year} 至今` : info.yoy == null ? '持平' : info.yoy > FLAT_BAND ? `▲${Math.round(info.yoy * 100)}%` : info.yoy < -FLAT_BAND ? `▼${Math.abs(Math.round(info.yoy * 100))}%` : '持平';
    return `${name} · ${fmtInt(Math.round(info.raw))}${unit} · ${arrow}`;
  }
  ...
  _build() {
    ...
    const light = !!this.layers._themeLight; const fill = light ? '#1E2939' : '#F3F4F6', ink = light ? '#FFFFFF' : '#030712', bg = light ? '#FFFFFF' : '#030712';
  ...
  _patchHeatLabels() {
    const ds = this.layers.ds.heat; if (!ds) return; const self = this;
  ...
  cityStats(year) { const idx = this._idxForYear(year); const y = this.years[idx] ?? year; const ytd = y === this._ytdYear;
    const salesAllArr = this.citySeriesFor('sales_all');
    ...
    const yoyPct = (!ytd && idx > 0 && salesAllArr[idx - 1]) ? (salesAllArr[idx] - salesAllArr[idx - 1]) / salesAllArr[idx - 1] : null;
    return { salesAll, salesOffice, licenses, yoyPct, ytd, year: y }; }
  districtStats(name, year) { return this._valueInfo(this.metric, name, year); }
  topMover(year) { let best = null; for (const d of this._districts) { const info = this._valueInfo(this.metric, d.name, year); if (info.yoy == null) continue; if (!best || Math.abs(info.yoy) > Math.abs(best.yoyPct)) best = { name: d.name, yoyPct: info.yoy, value: Math.round(info.raw) }; } return best; }
  ...
  get api() {
    ...
    return {
      ...
      get source() { return self._src; }, get ytdYear() { return self._ytdYear; },
      cityStats: y => self.cityStats(y), districtStats: (n, y) => self.districtStats(n, y), topMover: y => self.topMover(y), isInsufficient: (m, y) => self.isInsufficient(m, y),
      ready: self._ready,
    };
  }
}
