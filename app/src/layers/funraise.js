...
export const LAYERS = {
  ...
  future:   { name: '未來供給（規劃中）', color: '#93DCE6', glyph: 'ghost',   desc: '興建中／規劃中建案 · 逐層用途 · 完工年（隨時間軸長高）' },
  licenses: { name: '建照（即將開工）',   color: '#50C0D4', glyph: 'pulse',   desc: '臺北市 114–115 年建照 · 未來 24–48 月新供給訊號' },
  renewal:  { name: '都更單元',        color: '#C4B5FD', glyph: 'polygon', desc: '都更地區／單元圖形 · 政府主導／已核定事業' },
  zones:    { name: '重劃／區段徵收',   color: '#A78BFA', glyph: 'hex',     desc: '臺北市 73 筆市地重劃／區段徵收' },
  mops:     { name: '上市櫃資產交易',   color: '#F29628', glyph: 'diamond', desc: '公開資訊觀測站 取得／處分資產（近 12 月）' },
  moves:    { name: '企業遷徙',        color: '#DE7020', glyph: 'arc',     desc: '公司登記地址跨區異動（2026-07）· 由原址飛向新址' },
  infra:    { name: '公共建設（興建中）', color: '#BBEAF0', glyph: 'square',  desc: '捷運環狀線／信義東延／汐東線 · TOD 開發' },
  parks:    { name: '產業園區',        color: '#6EE7B7', glyph: 'ring',    desc: '產業園區範圍' },
  heat:     { name: '商圈行情',        color: '#FCBE83', glyph: 'heat',    desc: '商圈租金／售價熱度' },
  mrt:      { name: '捷運路網',        color: '#99A1AF', glyph: 'line',    desc: '台北捷運 6 線（OSM）' },
};
...
export class FunraiseLayers {
  ...
  buildLicenses() {
    for (const l of this.d.building_licenses || []) {
      ...
      this.add('licenses', key, l, { position: Cesium.Cartesian3.fromDegrees(l.lon, l.lat, 6), show: new Cesium.CallbackProperty(() => this.year >= (y || 2025), false),
        point: { pixelSize: new Cesium.CallbackProperty(() => 7 + 4 * (0.5 + 0.5 * Math.sin(this.t * 2.2 + ph)) + (this.isHot(key) ? 6 : 0), false), color: C('#50C0D4', .95), outlineColor: C('#50C0D4', .25), outlineWidth: new Cesium.CallbackProperty(() => 6 + 8 * (0.5 + 0.5 * Math.sin(this.t * 2.2 + ph)), false), disableDepthTestDistance: Number.POSITIVE_INFINITY },
  ...
  buildRenewal() {
    for (const u of this.d.urban_renewal || []) {
      ...
      if (n) { u._c = [cx / n, cy / n]; this.ds.labels.entities.add({ show: new Cesium.CallbackProperty(() => this.vis.renewal, false), position: Cesium.Cartesian3.fromDegrees(cx / n, cy / n, 10), label: label(`${u.name}`, { color: '#DDD6FE', far: 2600, dy: -8 }), properties: { pl: { layer: 'renewal', key, item: u } } }); }
    }
  }
  /* ---- 重劃／區段徵收 ---- */
  buildZones() {
  ...
  buildMops() {
    for (const m of this.d.mops || []) { if (!m.lat) continue; const key = 'mops:' + m.id; const y = yearOf(m.announcement_date); m._year = y; const size = Math.max(9, Math.min(26, 6 + Math.log10(Math.max(1e6, m.total_price || 1e6)) * 2.2));
      this.add('mops', key, m, { position: Cesium.Cartesian3.fromDegrees(m.lon, m.lat, 14), show: new Cesium.CallbackProperty(() => y == null || (y <= this.year && y >= this.year - 1), false),
        point: { pixelSize: new Cesium.CallbackProperty(() => size + (this.isHot(key) ? 8 * (0.5 + 0.5 * Math.sin(this.t * 5)) : 0), false), color: C('#F29628', .92), outlineColor: C('#FFDAA0', .9), outlineWidth: 2, disableDepthTestDistance: Number.POSITIVE_INFINITY },
  ...
  buildMoves() {
    ...
    for (const mv of this.d.registry_moves || []) { if (!mv.lat) continue; const key = 'move:' + mv.uniform_number; const from = orig(mv.before); if (!from) continue; const to = [mv.lon, mv.lat]; const pts = arc(from, to, 44); mv._from = from;
      ...
      this.ds.moves.entities.add({ position: Cesium.Cartesian3.fromDegrees(to[0], to[1], 30), label: label(mv.company_name.replace(/股份有限公司|有限公司/, ''), { color: '#FFDAA0', far: 2200, font: MONO, dy: -10 }), properties: { pl: { layer: 'moves', key, item: mv } } }); }
  }
  originFor(before) {
  ...
  buildMrt() {
    ...
    for (const s of this.base.mrt_stations || []) { const key = 'mrt:' + s.name; this.add('mrt', key, s, { position: Cesium.Cartesian3.fromDegrees(s.lon, s.lat, 4), point: { pixelSize: 6, color: C('#030712'), outlineColor: C('#D1D5DB'), outlineWidth: 2, disableDepthTestDistance: Number.POSITIVE_INFINITY, distanceDisplayCondition: new Cesium.DistanceDisplayCondition(0, 14000) }, label: label(s.name, { color: '#D1D5DB', far: 2800, font: '11px "Noto Sans TC", sans-serif', dy: -8 }) }); }
  }
  buildDistricts() {
