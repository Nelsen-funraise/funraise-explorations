...
async function testKey(key, env, getLLM) {
  ...
  try {
    switch (KEYS.find(x => x.k === key)?.test) {
      ...
      case 'ors': { if (!env.ORS_API_KEY) return { ok: false, detail: '未設定' }; const { r, j } = await t('https://api.openrouteservice.org/v2/isochrones/foot-walking', { method: 'POST', headers: { authorization: env.ORS_API_KEY, 'content-type': 'application/json' }, body: JSON.stringify({ locations: [[121.5645, 25.0339]], range: [60] }) }); return r.ok ? { ok: true, detail: `等時圈可用（${j && j.features ? j.features.length : 0} 個多邊形）` } : { ok: false, detail: `HTTP ${r.status} ${(j && j.error && (j.error.message || j.error)) || ''}`.trim() }; }
